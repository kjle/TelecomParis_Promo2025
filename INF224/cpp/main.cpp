...
int main(int argc, const char* argv[])
{
    ...
    // step 5
    Multimedia **mtab = new Multimedia*[2];
    int cnt = 0;
    mtab[cnt++] = new Photo("test.png", "./ressource/", 10, 10);
    mtab[cnt++] = new Video("test.mp4", "./ressource/", 10);
    for (int i = 0; i < cnt; i++) {
        mtab[i]->display(std::cout);
        mtab[i]->play();
    }
    ...
    shared_ptr<Group> group = manager->createGroup("testgroup");
    group->insertObject(photo);
    group->insertObject(video);
    group->insertObject(film);
    ...
    new TCPServer( [&](std::string const& request, std::string& response) {
        ...
        if (cmd == "Search" || cmd == "search") {
            ...
            if (arg2 == "Group" || arg2 == "group") {
                ...
                manager->findGroup(responseStream, groupName);
                response = responseStream.str();
            } else {
                manager->findMultimedia(responseStream, arg2);
                response = responseStream.str();
            }
